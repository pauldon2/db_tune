...
#	chmod 600 $PG_CONF
		...
		sed -i -r -E "s/^#?shared_buffers = [0-9]*[a-zA-Z]*/shared_buffers = ${SHARED_BUFFERS}MB/g" $PG_CONF

		MAX_CONNECTIONS=`grep "max_connections =" "$PG_CONF" | awk '{print $3}'`

		WORK_MEM=$(( DB_RAM * 1024 / MAX_CONNECTIONS / 2 ))

		sed -i -r -E "s/^#?work_mem = [0-9]*[a-zA-Z]*/work_mem = ${WORK_MEM}kB/" $PG_CONF

		MAX_PREP_TRANS=$(( MAX_CONNECTIONS / 10 ))

		echo "$MAX_PREP_TRANS"

		sed -i -r -E "s/^#?max_prepared_transactions = [0-9]*[a-zA-Z]*/max_prepared_transactions = ${MAX_PREP_TRANS}/g" $PG_CONF
		...
		sed -i -r -E "s/^#?maintenance_work_mem = [0-9]*[a-zA-Z]*/maintenance_work_mem = ${MAINTENANCE_WORK_MEM}MB/g" $PG_CONF

		MAX_STACK_DEPTH=$(( `ulimit -s` - 512 ))
		sed -i -r -E "s/^#?max_stack_depth = [0-9]*[a-zA-Z]*/max_stack_depth = ${MAX_STACK_DEPTH}kB/g" $PG_CONF

		EFFECTIVE_CACHE_SIZE=$(( DB_RAM / 4 * 3 ))
		sed -i -r -E "s/^#?effective_cache_size = [0-9]*[a-zA-Z]*/effective_cache_size = ${EFFECTIVE_CACHE_SIZE}MB/g" $PG_CONF
