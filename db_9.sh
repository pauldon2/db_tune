...
#	chmod 600 $PG_CONF
		...
		sed -i "s/#maintenance_work_mem = [0-9]*[a-zA-Z]*/maintenance_work_mem = ${MAINTENANCE_WORK_MEM}MB/g" $PG_CONF
		sed -i "s/maintenance_work_mem = [0-9]*[a-zA-Z]*/maintenance_work_mem = ${MAINTENANCE_WORK_MEM}MB/g" $PG_CONF

		MAX_STACK_DEPTH=$(( `ulimit -s` - 512 ))
		sed -i "s/#max_stack_depth = [0-9]*[a-zA-Z]*/max_stack_depth = ${MAX_STACK_DEPTH}kB/g" $PG_CONF
		sed -i "s/max_stack_depth = [0-9]*[a-zA-Z]*/max_stack_depth = ${MAX_STACK_DEPTH}kB/g" $PG_CONF

		EFFECTIVE_CACHE_SIZE=$(( DB_RAM / 4 * 3 ))
		sed -i "s/#effective_cache_size = [0-9]*[a-zA-Z]*/effective_cache_size = ${EFFECTIVE_CACHE_SIZE}MB/g" $PG_CONF
		sed -i "s/effective_cache_size = [0-9]*[a-zA-Z]*/effective_cache_size = ${EFFECTIVE_CACHE_SIZE}MB/g" $PG_CONF
