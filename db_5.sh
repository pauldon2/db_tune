...
PGDATA=/root/postgresql/$version/main
...
tunedb() {
	...
	if [[ ! -f $PGDATA/postgresql.conf.org ]]
	then
		echo_failure
		SCRIPT_RESULT=1
		echo
...
#For test Only
		cp $PGDATA/postgresql.conf.org $PGDATA/postgresql.conf 
...
#	chmod 600 $PG_CONF
		...
		sed -i "s/#e = 4GB/effective_cache_size = ${EFFECTIVE_CACHE_SIZE}MB/g" $PG_CONF
